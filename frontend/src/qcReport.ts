...
export const QC_REPORT_VERSION = "1.0.0";
export const QC_GENERATOR_VERSION = "0.1.0";
...
export type QcReport = {
  ...
  image: {
    source_kind: "local" | "iiif" | null;
    name: string | null;
    width: number | null;
    height: number | null;
    tile_source_url: string | null;
    local_fingerprint: FileFingerprint | null;
  };
  iiif: null | {
    loaded_url: string | null;
    kind: IiifInspection["kind"];
    version: IiifInspection["version"];
    id: string;
    selected_canvas_id: string | null;
...
export function buildQcReport(input: QcReportInput): QcReport {
  ...
  return {
    ...
    document: {
      ...
      pages: input.document.pages.map((page, index) => {
        ...
        return {
          ...
          width: page.width,
          height: page.height,
          measurement_unit: page.measurement_unit,
    ...
    iiif: input.iiif.inspection ? {
      ...
      canvas_dimensions: resolvedIiif?.canvas ? {
        width: resolvedIiif.canvas.width,
        height: resolvedIiif.canvas.height,
      } : null,
...
export function qcReportFilename(input: QcReport): string {
  const stem = input.document.xml_fingerprint?.name
    ?.replace(/\.xml$/i, "")
